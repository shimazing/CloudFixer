    ...
    def __init__(
        ...
        ***
        ...
        if schedule == 'discrete':
            ...
            self.t_array = torch.linspace(0., 1., self.total_N).reshape((1, -1)).to(dtype=dtype)
...
def model_wrapper(
    ...
    def get_model_input_time(t_continuous):
        ...
        if noise_schedule.schedule == 'discrete':
            return t_continuous * (noise_schedule.total_N - 1)
            #return ((t_continuous - 1. / noise_schedule.total_N) *
            #        1000.).long().float()
        else:
            return t_continuous
    ...
    def noise_pred_fn(x, t_continuous, cond=None):
        t_input = get_model_input_time(t_continuous)
        if cond is None:
            output = model(x, t_continuous, #t_input,
                    **model_kwargs)
        else:
            output = model(x, t_continuous, #t_input,
                    cond, **model_kwargs)
...
class DPM_Solver:
    ...
    def inverse(self, x, steps=20, t_start=None, t_end=None, order=2, skip_type='time_uniform',
        ...
        t_0 = 0 if t_start is None else t_start
    ...
    def sample(self, x, steps=20, t_start=None, t_end=None, order=2, skip_type='time_uniform',
        ...
        t_0 = 0 if t_end is None else t_end
