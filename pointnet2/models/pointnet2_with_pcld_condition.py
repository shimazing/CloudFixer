...
class PointNet2CloudCondition(PointNet2SemSegSSG):
    ...
    def _build_model(self):
        ...
        if self.include_global_feature:
            ...
            self.global_pnet = Pnet2Stage(self.hparams['pnet_global_feature_architecture'][0],
                                            ...
                                            include_t=self.hparams.get('pnet_include_t', False),
                                            t_dim=t_dim*4)
        ...
        if self.include_local_feature:
            ...
            if self.hparams['scale'] > 1:
                radius_condition = [r*self.hparams['scale'] for r in radius_condition]
            ...
            self.SA_modules_condition = self.build_SA_model(npoint_condition, radius_condition,
                                    ...
                                    radius_normalize=self.hparams['radius_normalize'],
                                    )
            ...
            if self.hparams['scale'] > 1:
                encoder_radius_feature_map = [r*self.hparams['scale'] for r in
                        encoder_radius_feature_map]
        ...
        if self.hparams['scale'] > 1:
            radius = [r*self.hparams['scale'] for r in radius]
        ...
        self.SA_modules = self.build_SA_model(npoint, radius,
                                ...
                                radius_normalize=self.hparams['radius_normalize'],
                                add_feature_level=self.hparams.get('add_feature_level',
                                    False),
                                )
        ...
        if self.include_local_feature:
            ...
            if self.hparams['scale'] > 1:
                decoder_radius_feature_map = [r*self.hparams['scale'] for r in
                        decoder_radius_feature_map]
        ...
        self.FP_modules = self.build_FP_model(decoder_feature_dim, decoder_mlp_depth, feature_dim, in_fea_dim,
                        ...
                        include_global_feature=self.include_global_feature, global_feature_dim=self.global_feature_dim,
                                                ...
                                                radius_normalize=self.hparams['radius_normalize'],
                                                global_attention_setting=self.global_attention_setting,
                                                add_feature_level=self.hparams.get('add_feature_level', False),
                                                use_radius=self.hparams.get('use_radius_dec',False),
                                                )
    ...
    def forward(self, pointcloud, condition, ts=None, label=None,
            use_retained_condition_feature=False, zero_mean=False):
        r"""
        ...
        if self.include_global_feature:
            ...
            if use_retained_condition_feature:
                if self.global_feature is None:
                    global_feature = self.global_pnet(global_input.transpose(1,2), t_emb)
        ...
        if self.concate_partial_with_noisy_input:
            out = out[:,0:N1,:]
        if zero_mean:
            out = out - out.mean(dim=1, keepdim=True) # zero centering
        return out
