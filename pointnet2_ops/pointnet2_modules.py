...
class _PointnetSAModuleBase(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor, features: Optional[torch.Tensor],
        ...
        for i in range(len(self.groupers)):
            ...
            if self.add_feature_level:
                grouped_features_feat = knn_points(new_xyz_feat.transpose(1,2),
                        features.transpose(1,2), K=self.nsamples[i] // 4,
                        return_nn=True)[2].permute(0,3,1,2)
                count_feat = 'all' #count.new_ones(len(new_xyz_feat))*(self.nsamples[i]//4)
            ...
            # (B, mlp[-1], npoint, K)
            if self.add_feature_level:
                out_features_feat = self.mlps_feat[i](grouped_features_feat,
            ...
            if self.use_attention_module:
                ...
                # (B, mlp[-1], npoint)
                if self.add_feature_level:
                    new_features_feats = self.attention_modules_feat[i](new_xyz_feat,
                            grouped_features_feat, out_features_feat,
                            count_feat)
            else:
                new_features = pooling_features(out_features, count=count, pooling=pooling)
                new_features_feats = pooling_features(out_features_feat,
                        count=count_feat, pooling=pooling)
            if self.add_feature_level:
                new_features = torch.cat((new_features, new_features_feats),
                        dim=1)
                new_features = self.combine_conv[i](new_features)
...
class PointnetSAModuleMSG(_PointnetSAModuleBase):
    ...
    def __init__(self, npoint, radii, nsamples, mlps, bn=True, use_xyz=True,
                        ...
                        radius_normalize=True, add_feature_level=False):
        ...
        self.add_feature_level = add_feature_level # 본래 좌표 + feature level의proximity도고려하는지
        ...
        if add_feature_level:
            self.mlps_feat = nn.ModuleList()
            if self.use_attention_module:
                self.attention_modules_feat = nn.ModuleList()
            self.combine_conv = nn.ModuleList()
        ...
        for i in range(len(radii)):
            ...
            self.groupers.append(
                pointnet2_utils.QueryAndGroup(radius, nsample, use_xyz=use_xyz,
                                            ...
                                            radius_normalize=radius_normalize)
            ...
            ori_mlp_spec = copy.deepcopy(mlp_spec)
            ...
            if add_feature_level:
                self.mlps_feat.append(Mlp_plus_t_emb(ori_mlp_spec, bn, t_dim=self.t_dim, include_t=include_t,
                            bn_first=bn_first, bias=bias, first_conv=first_conv,
                            first_conv_in_channel=first_conv_in_channel, res_connect=res_connect,
                            include_condition=include_condition, condition_dim=condition_dim,
                            include_second_condition=include_second_condition, second_condition_dim=second_condition_dim,
                            activation=activation))
                self.combine_conv.append(nn.Conv1d(2*mlp_spec[-1],mlp_spec[-1],
                    1))
            ...
            if self.use_attention_module:
                ...
                if add_feature_level:
                    self.attention_modules_feat.append(AttentionModule(ori_mlp_spec0,
                        ori_mlp_spec0, ori_mlp_spec0, ori_mlp_spec0, C_out,
                        attention_bn=attention_setting['attention_bn'],
                        transform_grouped_feat_out=attention_setting['transform_grouped_feat_out'],
                        last_activation=attention_setting['last_activation']))
...
class PointnetSAModule(PointnetSAModuleMSG):
    ...
    def __init__(self, mlp, npoint=None, radius=None, nsample=None, bn=True, use_xyz=True,
        ...
        super(PointnetSAModule, self).__init__(
            ...
            add_feature_level=add_feature_level,
        )
...
class PointnetFPModule(nn.Module):
    ...
    def __init__(self, mlp, bn=True, t_dim=128, include_t=False,
        ...
        self.include_second_condition = include_second_condition
        self.second_condition_dim = second_condition_dim
        self.radius_normalize = radius_normalize
...
class FeatureMapModule(nn.Module):
    def __init__(self, mlp, radius, K, use_xyz=True, include_abs_coordinate=True, include_center_coordinate=False,
                    ...
                    query_feature_dim=None, radius_normalize=True):
        # type: (PointnetFPModule, List[int], bool) -> None
...
class PointnetKnnFPModule(nn.Module):
    ...
    def __init__(self, mlp1, mlp2, K, bn=True, t_dim=128, include_t=False,
        ...
        self.add_feature_level = add_feature_level
        self.use_radius = use_radius
        mlp1_ori = copy.deepcopy(mlp1)
        if use_radius:
            self.radius_grouper = QueryAndGroup(radius*2, nsample,
                    use_xyz=use_xyz,
                    include_abs_coordinate=include_abs_coordinate,
                    include_center_coordinate=include_center_coordinate,
                    neighbor_def='radius',
        ...
        if add_feature_level: # unknown (C1)
            self.mlp1_feat = Mlp_plus_t_emb([mlp2[0] - mlp1[-1]] + mlp1_ori[1:], bn, t_dim=self.t_dim, include_t=False,
                            bn_first=bn_first, bias=bias, first_conv=first_conv,
                            first_conv_in_channel=first_conv_in_channel1, res_connect=res_connect,
                            include_condition=include_second_condition, condition_dim=second_condition_dim,
                            activation=activation)
            self.combine_conv = nn.Conv1d(mlp1[-1]*2, mlp1[-1], 1)
        ...
        if self.use_attention_module:
            ...
            self.attention_module = AttentionModule(C_in1, C_in2, C1, C2, C_out,
                ...
                last_activation=attention_setting['last_activation'])
            if add_feature_level:
                self.attention_module_feat = AttentionModule(C_in1, C_in1, C1, C1, C_out,
                    attention_bn=attention_setting['attention_bn'],
                    transform_grouped_feat_out=attention_setting['transform_grouped_feat_out'],
                    last_activation=attention_setting['last_activation'])
    ...
    def forward(self, unknown, known, unknow_feats, known_feats, t_emb=None, condition_emb=None,
        ...
        if known is not None:
            ...
            #print("known", known.shape)
            #print("known_feats", known_feats.shape)

            if self.use_radius:
                grouped_feats, count = self.radius_grouper(known, unknown,
                        known_feats, subset=False, return_counts=True)
            else:
                count = None
            ...
            if self.use_attention_module:
                ...
                interpolated_feats = self.attention_module(unknow_feats,
                        grouped_feats, grouped_feats_out, count='all' if count
                        is None else count)
                # (B, mlp1[-1], n)
            else:
                interpolated_feats = pooling_features(grouped_feats_out,
                        count='all' if count is None else count, pooling=pooling)
                # (B, mlp1[-1], n)
        ...
        if self.add_feature_level:
            grouped_feats_feat = knn_points(unknow_feats.transpose(1,2), unknow_feats.transpose(1,2),
                    K=self.K, return_nn=True)[2].permute(0,3,1,2)
            second_condition_emb = second_condition_emb if self.include_second_condition else None
            grouped_feats_out_feat = self.mlp1_feat(grouped_feats_feat,
                    t_emb=None, condition_emb=second_condition_emb)

            if self.use_attention_module:
                # unknow_feats (B, C1, n)
                # unknow_feats is used as query
                interpolated_feats_feat = self.attention_module_feat(unknow_feats,
                        grouped_feats_feat, grouped_feats_out_feat, count='all')
                # (B, mlp1[-1], n)
            else:
                interpolated_feats_feat = pooling_features(grouped_feats_out_feat, count='all', pooling=pooling)
                # (B, mlp1[-1], n)
            interpolated_feats = self.combine_conv(
                    torch.cat((
                        interpolated_feats,
                        interpolated_feats_feat), dim=1))
        if unknow_feats is not None:
