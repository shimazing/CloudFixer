...
class ModelNet40C(Dataset):
    ...
    def load_modelnet40_c(self, data_path='data/modelnet40_c', corruption='cutout', severity=1, num_classes=40):
        if corruption == 'original':
            ...
            data_dir_list = [os.path.join(data_path, f'data_{corruption}_{severity}.npy') for corruption in corruption_list]
            all_data_list = [np.load(data_dir) for data_dir in data_dir_list]
            selected_indices = np.random.choice(len(corruption_list), len(all_data_list[0]), replace=True)
            all_data = []
