...
class Point2VecClassification(pl.LightningModule):
    def __init__(
        ...
        transformation_scale_min: float = 0.6,
        transformation_scale_max: float = 1.4,
        ...
        if vote:
            self.val_vote_acc = Accuracy(task='multiclass', num_classes=40)

        self.val_top_3_acc = Accuracy(task='multiclass',top_k=3, num_classes=40)
    ...
    def setup(self, num_classes=40, stage: Optional[str] = None) -> None:
        if self.hparams.cls_head == "mlp":  # type: ignore
            self.cls_head = nn.Sequential(
                ...
                nn.ReLU(),
                nn.Dropout(self.hparams.cls_head_dropout),  # type: ignore
                nn.Linear(self.hparams.cls_head_dim, num_classes),  # type: ignore
            )
        elif self.hparams.cls_head == "linear":  # type: ignore
            self.cls_head = nn.Linear(self.first_cls_head_dim, num_classes)  # type: ignore

        self.val_macc = Accuracy(task='multiclass',num_classes=num_classes,
    ...
    def forward(self, points: torch.Tensor) -> torch.Tensor:
        ...
        points = scale_to_unit_cube_torch(points)
        points = rotate_shape_tensor(points, 'x', np.pi/2)
