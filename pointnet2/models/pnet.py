...
class Pnet2Stage(nn.Module):
    def __init__(self, mlp1, mlp2, bn=True, remove_last_activation=True,
            include_t=False, t_dim=128):
        super().__init__()
        self.include_t = include_t
        self.mlp1 = Mlp_plus_t_emb(mlp1, bn=bn, t_dim=t_dim, include_t=include_t,
        ...
        mlp2 = [2*mlp1[-1]] +  mlp2
        self.mlp2 = Mlp_plus_t_emb(mlp2, bn=bn, t_dim=t_dim, include_t=include_t,
                        bn_first=False, bias=True, first_conv=False,
                        first_conv_in_channel=0,
                        res_connect=False,
                        include_condition=False, condition_dim=128)
    ...
    def forward(self, x, t_emb=None):
        ...
        feature = self.mlp2(feature, t_emb=t_emb)
        global_feature = F.max_pool2d(feature, kernel_size=[feature.size(2), 1])
