...
class DiffusionModel(torch.nn.Module):
    ...
    def kl_prior(self, xh, node_mask):
        ...
        if node_mask is None:
            node_mask = torch.ones(xh.shape[:-1] + (1,)).to(xh.device)
        ...
        sigma_T_x = self.sigma(gamma_T, mu_T_x).squeeze(dim=-1).squeeze(dim=-1)  # Remove inflate, only keep batch dimension for x-part.
    ...
    def sample_p_x_given_z0(self, z0, node_mask, edge_mask,
            fix_noise=False, ddim=False):
        """Samples x ~ p(x|z0)."""
        zeros = torch.zeros(size=(z0.size(0), 1), device=z0.device)
        gamma_0 = self.gamma(zeros)
        # Computes sqrt(sigma_0^2 / alpha_0^2)
        sigma_x = self.SNR(-0.5 * gamma_0).unsqueeze(1)
        net_out = self.phi(z0, zeros, node_mask, edge_mask)
    ...
    def compute_loss(self, x, h, node_mask, edge_mask, t0_always):
        ...
        if t0_always: # EVAL
            ...
            net_out = self.phi(z_0, t_zeros, node_mask, edge_mask)
    ...
    def forward(self, x, h, node_mask=None, edge_mask=None):
        ...
        if True: #self.training:
            ...
            loss, loss_dict = self.compute_loss(x, h, node_mask, edge_mask, t0_always=True)
    ...
    def sample_combined_position_feature_noise(self, n_samples, n_nodes,
        ...
        if self.zero_mean:
            z_x = utils.sample_center_gravity_zero_gaussian_with_mask(
                ...
                if node_mask is not None else device,
                node_mask=node_mask)
        else:
            z_x = torch.randn((n_samples,n_nodes, self.n_dims),
                    device=node_mask.device)
        z = z_x
        return z

    @torch.no_grad()
    def sample(self, n_samples, n_nodes, node_mask, edge_mask, fix_noise=False):
        """
        ...
        x = self.sample_p_x_given_z0(z, node_mask, edge_mask, fix_noise=fix_noise)
        if self.zero_mean:
    ...
    @torch.no_grad()
    def sample_chain(self, n_samples, n_nodes, node_mask, edge_mask, keep_frames=None):
        """
        ...
        for s in reversed(range(0, self.T)):
            ...
            z = self.sample_p_zs_given_zt(
                s_array, t_array, z, node_mask, edge_mask)
            if self.zero_mean:
                diffusion_utils.assert_mean_zero_with_mask(z[:, :, :self.n_dims], node_mask)

            # Write to chain tensor.
            write_index = (s * keep_frames) // self.T
            chain[write_index] = z[:, :, :self.n_dims]

        # Finally sample p(x, h | z_0).
        x = self.sample_p_x_given_z0(z, node_mask, edge_mask)
        if self.zero_mean:
